...
1. **文本扭曲验证码**：早期形态，通过扭曲文字抗 OCR 原理。
2. **图片选择验证码**：如"选出所有红绿灯"，基于语义识别抗自动化。
3. **滑块验证码**：基于轨迹采集与服务端的轨迹特征校验。
4. **拼图验证码**：结合图像处理与空间拖动。
5. **点选文字验证码**：乱序点击特定文字。
6. **短信/语音验证码**：一次性验证码的另一种呈现形式，原理见 OTP 家族。
7. **无感验证码（reCAPTCHA v3 / Turnstile）**：行为打分机制原理，用户无感知。
8. **蜜罐技术（Honeypot）**：通过隐藏表单等手段诱导机器填写，反爬虫辅助手段。

### 2.2 滑块验证码交互校验流程

```mermaid
flowchart TD
    UserSlide[1. 用户拖动滑块] --> CollectTraj[2. 客户端高频采集拖动轨迹 x, y, time]
    CollectTraj --> SubmitData[3. 提交加密的轨迹特征与环境指纹至服务端]
    
    subgraph 服务端风控引擎
        SubmitData --> CheckSpeed{4. 校验速度与加速度合法性}
        CheckSpeed -- 匀速/非人类突变 --> Block[5. 拦截 / 判定为机器特征]
        CheckSpeed -- 符合人类操作 --> CheckCurve{6. 校验微小抖动与曲率}
        CheckCurve -- 完美直线 --> Block
        CheckCurve -- 自然抖动 --> Pass[7. 校验通过]
    end
    
    Pass --> ReturnToken[8. 颁发凭证 Token 放行请求]
```
<div class="diagram-caption">图 1-32a：滑块验证码轨迹采集与特征分析流程图</div>
...
```
<div class="diagram-caption">图 1-34：传统有感验证与现代无感验证的用户体验与安全性权衡</div>
